...
const Photo = z.object({ mimeType:z.enum(['image/jpeg','image/png','image/webp']),data:z.string().max(4194304) }).strict();
...
async function route(request: Request,context: Context) {
  try {
    ...
    if (method === 'GET' && path.join('/') === 'me') result = viewer;
      ...
      if (method !== 'GET' || tail[0] === 'inventory' || tail[0] === 'friends' || tail[0] === 'attempts') invariant(userId === viewer.id,403,'FORBIDDEN','You can only change your own progress.');
      if (method === 'GET' && tail.length === 0) result = await players.getPlayerProfile(userId);
        ...
        const body = z.object({ locationCode:LocationCode,attemptId:UUID.optional() }).strict().parse(await readJson(request));
        result = await quests.completeQuestByLocation(userId,body.locationCode,body.attemptId);
      }
        ...
        result = await quests.submitPhoto(userId,UUID.parse(tail[1]),decodePhoto(photo.data,photo.mimeType),photo.mimeType,verifyPhoto);
