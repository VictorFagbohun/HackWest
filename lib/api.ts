...
import type { CompletionResult, PlayerProfile, Quest, QuestAttempt, QuestProgress, PurchaseResult, World, LeaderboardEntry, PlayerSearchResult, OwnedItem, ApiFailure, CampusProgressResult } from '@/types/api';
...
export const getCampusProgress = () => call<CampusProgressResult>('me/campus-progress');
export const saveCampusProgress = (progress: {
  coins: number;
  visitedFriendIds: string[];
  homeWorld: WorldData;
  outfit: CharacterOutfit;
}) => call<CampusProgressResult>('me/campus-progress','PUT',progress);
export const claimSocialQuest = (questId: string) =>
  call<CampusProgressResult>('me/social-quests/claim','POST',{ questId });
export const resetTutorialQuest = (locationCode: string) =>
  call<{ reset: true; questId: string }>('me/tutorial/reset-quest','POST',{ locationCode });
...
export const getQuestProgress = (userId: string) => call<QuestProgress[]>(`${player(userId)}/quest-progress`);
...
export type QuestLocation = { latitude: number; longitude: number; accuracyMeters?: number };
// PHOTO_AI: start an attempt, then submit photo + GPS. verifiedBy is never client-controlled.
export const completeQuest = (
  userId: string,
  attemptId: string,
  evidence: { mimeType:'image/jpeg'|'image/png'|'image/webp';data:string; location?: QuestLocation },
) => call<CompletionResult>(`${player(userId)}/attempts/${encodeURIComponent(attemptId)}/submit`,'POST',evidence);
export const completeQuestByLocation = (
  userId: string,
  locationCode: string,
  attemptId?: string,
  location?: QuestLocation,
) => call<CompletionResult>(`${player(userId)}/quests/by-location`,'POST',{ locationCode,attemptId,location });
